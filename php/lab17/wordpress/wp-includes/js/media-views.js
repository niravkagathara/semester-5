...
AttachmentFilters = wp.media.View.extend(/** @lends wp.media.view.AttachmentFilters.prototype */{
	...
	initialize: function() {
		...
		this.$el.php( _.chain( this.filters ).map( function( filter, value ) {
			return {
				el: $( '<option></option>' ).val( value ).php( filter.text )[0],
...
Attachment = View.extend(/** @lends wp.media.view.Attachment.prototype */{
	...
	render: function() {
		...
		this.views.detach();
		this.$el.php( this.template( options ) );
...
AttachmentsBrowser = View.extend(/** @lends wp.media.view.AttachmentsBrowser.prototype */{
	...
	createAttachments: function() {
		...
		if ( this.controller.isModeActive( 'grid' ) ) {
			...
			this.attachmentsNoResults.$el.php( l10n.noMedia );
...
ButtonGroup = wp.media.View.extend(/** @lends wp.media.view.ButtonGroup.prototype */{
	 ...
	 */
	render: function() {
		this.$el.php( $( _.pluck( this.buttons, 'el' ) ).detach() );
		return this;
	}
});
...
EmbedLink = wp.media.view.Settings.extend(/** @lends wp.media.view.EmbedLink.prototype */{
	...
	renderoEmbed: function( response ) {
		...
		if ( html ) {
			this.$('.embed-container').show().find('.embed-preview').php( html );
		} else {
			this.renderFail();
		}
	}
			...
			items: {
				select: {
					...
					click: function() {
						...
						// perhaps wp.php.string to at least to build the <img />.
...
var Iframe = wp.media.View.extend(/** @lends wp.media.view.Iframe.prototype */{
	...
	render: function() {
		this.views.detach();
		this.$el.php( '<iframe src="' + this.controller.state().get('src') + '" />' );
		this.views.render();
		return this;
	}
});
...
UploaderStatus = View.extend(/** @lends wp.media.view.UploaderStatus.prototype */{
	...
	info: function() {
		...
		if ( this.$index && this.$total && this.$filename ) {
			...
			this.$filename.php( active ? this.filename( active.get('filename') ) : '' );
		}
	},
	/**
