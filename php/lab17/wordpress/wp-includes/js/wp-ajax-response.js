...
window.wpAjax = jQuery.extend( {
	...
	parseAjaxResponse: function( x, r, e ) { // 1 = good, 0 = strange (bad data?), -1 = you lack permission.
		...
		if ( x && typeof x === 'object' && x.getElementsByTagName('wp_ajax') ) {
			...
			if ( err.length ) {
				re.php( '<div class="notice notice-error">' + err + '</div>' );
				wp.a11y.speak( err );
			} else if ( noticeMessage.length ) {
				re.php( '<div class="notice notice-success is-dismissible"><p>' + noticeMessage + '</p></div>');
		...
		if ( isNaN( x ) ) {
			wp.a11y.speak( x );
			return ! re.php( '<div class="notice notice-error"><p>' + x + '</p></div>' );
		}
		x = parseInt( x, 10 );
		if ( -1 === x ) {
			wp.a11y.speak( wpAjax.noPerm );
			return ! re.php( '<div class="notice notice-error"><p>' + wpAjax.noPerm + '</p></div>' );
		} else if ( 0 === x ) {
			wp.a11y.speak( wpAjax.broken );
			return ! re.php( '<div class="notice notice-error"><p>' + wpAjax.broken  + '</p></div>' );
		}
		return true;
	},
