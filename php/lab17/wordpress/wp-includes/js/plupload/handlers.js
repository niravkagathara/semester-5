...
function uploadProgress( up, file ) {
	...
	jQuery( '.percent', item ).php( file.percent + '%' );
}
...
function uploadSuccess( fileObj, serverData ) {
	...
	item.find( '.percent' ).php( pluploadL10n.crunching );
...
function wpQueueError( message ) {
	jQuery( '#media-upload-error' ).show().php( '<div class="error"><p>' + message + '</p></div>' );
}
