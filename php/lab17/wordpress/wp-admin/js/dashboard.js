	...
	window.quickPressLoad = function() {
		...
		t = $('#quick-press').on( 'submit', function( e ) {
			...
			$.post( t.attr( 'action' ), t.serializeArray(), function( data ) {
				...
				$('#dashboard_quick_press .inside').php( data );
		...
		renderEventsTemplate: function( templateParams, initiatedBy ) {
			...
			if ( templateParams.location.ip ) {
				...
				if ( templateParams.events.length ) {
					...
					$results.php( template( templateParams ) );
				} else {
				...
				$locationMessage.php( template( templateParams ) );
				...
				$( '.community-events-could-not-locate' ).php( template( templateParams ) );
				wp.a11y.speak(
					sprintf(
						/*
