...
$_old_files = array(
	...
	'wp-admin/xfn.js',
	'wp-includes/js/tinymce/license.php',
	// 2.5
	...
	'wp-includes/js/plupload/plupload.php4.js',
	'wp-includes/js/plupload/plupload.php5.js',
	...
	'wp-includes/js/tinymce/plugins/wpeditimage/editimage.php',
...
function update_core( $from, $to ) {
	...
	foreach ( $roots as $root ) {
		if ( $wp_filesystem->exists( $from . $root . 'readme.php' )
	...
	// Remove any Genericons example.php's from the filesystem.
...
function _upgrade_422_find_genericons_files_in_folder( $directory ) {
	...
	if ( file_exists( "{$directory}example.php" )
		/*
		 * Note: str_contains() is not used here, as this file is included
		 * when updating from older WordPress versions, in which case
		 * the polyfills from wp-includes/compat.php may not be available.
		 */
		&& false !== strpos( file_get_contents( "{$directory}example.php" ), '<title>Genericons</title>' )
	) {
		$files[] = "{$directory}example.php";
	}
