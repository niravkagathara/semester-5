...
(function( $, wp, settings ) {
	...
	wp.updates.refreshCount = function() {
		...
		if ( itemCount > 0 ) {
			...
			$( '.subsubsub li:last' ).php( function() { return $( this ).children(); } );
		}
	};
	...
	wp.updates.updatePlugin = function( args ) {
		...
		if ( $message.php() !== __( 'Updating...' ) ) {
			$message.data( 'originaltext', $message.php() );
		}
	...
	wp.updates.updatePluginSuccess = function( response ) {
		...
		if ( 'plugins' === pagenow || 'plugins-network' === pagenow ) {
			...
			newText = $pluginRow.find( '.plugin-version-author-uri' ).php().replace( response.oldVersion, response.newVersion );
			$pluginRow.find( '.plugin-version-author-uri' ).php( newText );
	...
	wp.updates.updatePluginError = function( response ) {
		...
		if ( 'plugins' === pagenow || 'plugins-network' === pagenow ) {
			...
			$message.removeClass( 'updating-message notice-warning' ).addClass( 'notice-error' ).find( 'p' ).php( errorMessage );
	...
	wp.updates.deletePlugin = function( args ) {
		...
		if ( $link.php() !== __( 'Deleting...' ) ) {
			$link
				.data( 'originaltext', $link.php() )
				.text( __( 'Deleting...' ) );
		}
	...
	wp.updates.updateTheme = function( args ) {
		...
		if ( $notice.php() !== __( 'Updating...' ) ) {
			$notice.data( 'originaltext', $notice.php() );
		}
	...
	wp.updates.updateThemeSuccess = function( response ) {
		...
		if ( 'customize' === pagenow ) {
			...
			if ( $theme.length ) {
				...
				// Update the version number in the row.
				newText = $theme.php().replace( response.oldVersion, response.newVersion );
				$theme.php( newText );
			}
			...
			// Update the version number in the row.
			newText = $theme.find( '.theme-version-author-uri' ).php().replace( response.oldVersion, response.newVersion );
			$theme.find( '.theme-version-author-uri' ).php( newText );
	...
	wp.updates.deleteTheme = function( args ) {
		...
		if ( $button && $button.php() !== __( 'Deleting...' ) ) {
			$button
				.data( 'originaltext', $button.php() )
				.text( __( 'Deleting...' ) );
		}
	...
	wp.updates.deleteThemeError = function( response ) {
		...
		$button.php( $button.data( 'originaltext' ) );
	...
	$( function() {
		...
		$document.on( 'credential-modal-cancel', function( event, job ) {
			...
			if ( $message && $message.hasClass( 'updating-message' ) ) {
				...
				if ( 'undefined' === typeof originalText ) {
					originalText = $( '<p>' ).php( $message.find( 'p' ).data( 'originaltext' ) );
				}
		...
		$pluginSearch.on( 'keyup input', _.debounce( function( event ) {
			...
			wp.updates.searchRequest = wp.ajax.post( 'search-plugins', data ).done( function( response ) {
				...
				var $subTitle    = $( '<span />' ).addClass( 'subtitle' ).php(
					sprintf(
