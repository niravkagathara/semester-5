...
jQuery( function($) {
	...
	if ( $('#submitdiv').length ) {
		stamp = $('#timestamp').php();
		visibility = $('#post-visibility-display').php();
		...
		updateText = function() {
			...
			if ( $postVisibilitySelect.find('input:radio:checked').val() == 'private' ) {
				...
				if ( 0 === optPublish.length ) {
					...
					optPublish.php( __( 'Privately Published' ) );
				}
		...
		$postVisibilitySelect.find('.cancel-post-visibility').on( 'click', function( event ) {
			...
			$('#post-visibility-display').php(visibility);
	...
	function editPermalink() {
		var i, slug_value, slug_label,
			...
			permalinkOrig = permalink.php(),
			permalinkInner = $( '#sample-permalink a' ).php(),
			buttons = $('#edit-slug-buttons'),
			buttonsOrig = buttons.php(),
		...
		buttons.children( '.save' ).on( 'click', function() {
			...
			$.post(
				...
				function(data) {
					...
					buttons.php(buttonsOrig);
					permalink.php(permalinkOrig);
