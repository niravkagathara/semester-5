...
( function( $, wp ) {
	...
	function configureLinting( editor, settings ) { // eslint-disable-line complexity
		...
		function getLintOptions() { // eslint-disable-line complexity
			...
			// Configure HTMLHint.
			if ( 'htmlmixed' === settings.codemirror.mode && settings.phphint ) {
				options.options.rules = $.extend( {}, settings.phphint );
	 ...
	 * @param {Object}                [settings.phphint] - Rules for HTMLHint.
			...
			codemirror.on( 'keyup', function( editor, event ) { // eslint-disable-line complexity
				...
				if ( 'html' === innerMode || 'xml' === innerMode ) {
					shouldAutocomplete =
						...
						'=' === token.string && token.state.phpState && token.state.phpState.tagName;
