...
(function($) {
	...
	action : function(postid, nonce, action) {
		...
		$.post( ajaxurl, data, function( response ) {
			$( '#image-editor-' + postid ).empty().append( response.data.php );
			t.toggleEditor( postid, 0, true );
		...
		$.post( ajaxurl, data, function( response ) {
			...
			if ( response.data.error ) {
				$( '#imgedit-response-' + postid )
					.php( '<div class="notice notice-error" tabindex="-1" role="alert"><p>' + response.data.error + '</p></div>' );
			...
			if ( response.data.fw && response.data.fh ) {
				$( '#media-dims-' + postid ).php( response.data.fw + ' &times; ' + response.data.fh );
			}
			...
			if ( response.data.msg ) {
				$( '#imgedit-response-' + postid )
					.php( '<div class="notice notice-success" tabindex="-1" role="alert"><p>' + response.data.msg + '</p></div>' );
	...
	open : function( postid, nonce, view ) {
		...
		dfd = $.ajax( {
			...
			if ( '-1' === response ) {
				...
				elem.php( '<div class="notice notice-error" tabindex="-1" role="alert"><p>' + errorMessage + '</p></div>' );
			}

			if ( response.data && response.data.php ) {
				elem.php( response.data.php );
			}
	...
	/**
	 * Validates if a value in a jQuery.phpElement is numeric.
	 *
	 * @since 4.6.0
	 *
	 * @memberof imageEdit
