...
window.wpWidgets = {
		...
		sidebars.sortable({
			...
			stop: function( event, ui ) {
				...
				if ( addNew ) {
					if ( 'multi' === addNew ) {
						$widget.php(
							$widget.php().replace( /<[^<>]+>/g, function( tag ) {
		...
		$('#available-widgets').droppable({
			...
			over: function(e,ui) {
				...
				if ( ui.draggable.hasClass('ui-sortable-helper') ) {
					...
					.php( ui.draggable.find( 'div.widget-title' ).children( 'h3' ).php() );
				}
			},
			out: function(e,ui) {
	...
	save : function( widget, del, animate, order ) {
		...
		$.post( ajaxurl, data, function(r) {
			...
			if ( del ) {
				...
				$( '.spinner' ).removeClass( 'is-active' );
				if ( r && r.length > 2 ) {
					$( 'div.widget-content', widget ).php( r );
	...
	appendTitle : function(widget) {
		...
		$(widget).children('.widget-top').children('.widget-title').children()
				.children('.in-widget-title').php(title);
	...
	addWidget: function( chooser ) {
		...
		if ( 'multi' === add ) {
			widget.php(
				widget.php().replace( /<[^<>]+>/g, function(m) {
