	...
	api = window.wpNavMenu = {
		...
		attachPendingMenuItemsListForDeletion : function() {
			$( '#post-body-content' ).on( 'change', '.menu-item-checkbox', function() {
				...
				$( '#pending-menu-items-to-delete li .separator' ).php( ', ' );
				$( '#pending-menu-items-to-delete li .separator' ).last().php( '.' );
			});
		},
			...
			$( '#nav-menu-meta' ).on( 'click', 'a.page-numbers', function() {
				...
				$.post( ajaxurl, this.href.replace( /.*\?/, '' ).replace( /action=([^&]*)/, '' ) + '&action=menu-get-metabox',
					function( resp ) {
						...
						$container.php( metaBoxData.markup );
					}
				);
		...
		processQuickSearchQueryResponse : function(resp, req, panel) {
			...
			$items.each(function(){
				...
				if ( matched && matched[1] ) {
					...
					takenIDs[newID] = true;
					if ( newID != matched[1] ) {
						$item.php( $item.php().replace(new RegExp(
			...
			$('.categorychecklist', panel).php( $items );
			$( '.spinner', panel ).removeClass( 'is-active' );
			wrapper.removeClass( 'has-no-menu-item' );
